...
imageDirRootPath=../../../big-files/customer/suzhoushenpiju/images/minerva
mkdir -p $imageDirRootPath/identity
mkdir -p $imageDirRootPath/expert
...
downloadAndSave() {
  ...
  then
    docker save $imageFullName | gzip > $imageDirRootPath/identity/$(echo $fullnameWithouCloudPrefix | sed "s#/#-#g; s#:#-#g").tgz
  else
    docker save $imageFullName | gzip > $imageDirRootPath/$category/$(echo $fullnameWithouCloudPrefix | sed "s#/#-#g; s#:#-#g").tgz
  fi
}
...
confirmToDownload "ui_approval" "latest" "expert"
confirmToDownload "api_approval" "latest" "expert"

confirmToDownload "metis_api" "latest" "expert"
confirmToDownload "db_approval" "latest" "expert"
confirmToDownload "metis_db" "latest" "expert"

confirmToDownload "metis_worker" "latest" "expert"
confirmToDownload "intelligence_mt" "metis" "expert"